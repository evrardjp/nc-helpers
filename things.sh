...
function host_jump {
  HOST=$1
  kubectl -n ucp exec -it "$(kubectl get -n ucp pods -l application=divingbell,component=sysctl -o wide --no-headers | awk "/${HOST}/ { print \$1; exit }")" -- nsenter -t 1 -m -u -n -i bash
}
